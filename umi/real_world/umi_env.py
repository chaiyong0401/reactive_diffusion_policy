...
# config_path = base_path / "digit-depth" / "config" / "digit.yaml"
# with open(config_path, 'r') as f:
#     cfg = yaml.safe_load(f)


# ## frame to depth related
# seed =42
...
# # config 값 사용
# fps = cfg['sensor']['fps']
# resolution = cfg['sensor']['resolution']
# serial_num = cfg['sensor']['serial_num']

# # @hydra.main(config_path=f"{base_path}/digit-depth/config", config_name="digit.yaml", version_base=None)
# digit_sensor = DigitSensor(fps,resolution,serial_num)
# # digit_sensor = DigitSensor(fps,resolution)
...
class UmiEnv:
    def __init__(self, 
        ...
        # self.latest_tactile_frame = None
        # self._tactile_thread = threading.Thread(
        #     target=self._update_tactile_loop,
        #     daemon=True
        # )
            ...
            if 'Cam_Link_4K' in path:
                ...
                def tf4k(data, input_res=res):
                    ...
                    f = get_image_transform(
                        ...
                        bgr_to_rgb=True)
                    # tactile overlay ##########################################
                    # tactile_frame = digit_call.get_frame()  # 현재 시점 촉각 이미지
                    # tactile_resized = cv2.resize(tactile_frame, (32, 32))
                    # # overlay
                    # h, w, _ = img.shape
                    # x_offset = 50 
                    # y_offset = h - 42
                    # img.frame[y_offset:y_offset+32, x_offset:x_offset+32] = tactile_resized
                    ###########################################################
                    img = f(img)
                    if obs_float32:
                ...
                def tf(data, input_res=res, stack_crop=stack_crop, is_mirror=is_mirror):
                    ...
                    # # x_offset = 50 
                    # # y_offset = h - 42
                    # # img[y_offset:y_offset+32, x_offset:x_offset+32] = tactile_resized

                    # # tactile depth iamge overlay
                    # tactile_frame = digit_call.get_frame()  # 현재 시점 촉각 이미지
                    # # tactile_frame = self.latest_tactile_frame
                    # if tactile_frame is None:
                    #     print("tactile frame nt ready yet, skipping overlay")
                    # img_np = preproc_mlp(tactile_frame)
                    ...
                    # dm_zero = np.load("/home/embodied-ai/mcy/digit-depth/dm_zero.npy")
                    ...
                    # current_depth = img_depth[y_idx, x_idx]  # (63,)
                    # initial_depth = dm_zero[y_idx, x_idx]    # (63,)

                    # marker_offset = current_depth - initial_depth  # (63,)

                    # marker_offset = marker_offset * 5000 * -1
                    # marker_offset[marker_offset < 0] = 0
                    # #############################################################################
                    # # 보정 및 저장
                    # diff = (img_depth - dm_zero) * 5000 * -1
                    ...
                    # tactile_resized = cv2.resize(norm_diff, (40, 40))

                    # # gray channel to rgb channel overlay
                    # tactile_rgb = cv2.cvtColor(tactile_resized, cv2.COLOR_GRAY2BGR)
                    # h, w, _ = img.shape
                    # x_offset = 20
                    # y_offset = h - 45
                    # img[y_offset:y_offset+40, x_offset:x_offset+40] = tactile_rgb
                    ###########################################################
                    if obs_float32:
                        img = img.astype(np.float32) / 255
    ...
    def exec_actions(self, 
        ...
        # print("umi_exec_actions[new_actions]: ", new_actions)
        # print("umi_exec_actions[new_timestamps]: ", new_timestamps[0])
    ...
    # def _update_tactile_loop(self):
    #     while True:
    #         try:
    #             self.latest_tactile_frame = digit_call.get_frame()
    #             frame = self.latest_tactile_frame
    #             # print("Tactile frame update:", time.time())
    #             # print("Tactile frame mean:", np.mean(frame), "std: ", np.std(frame))
    #         except Exception as e:
    #             print(f"[Tactile Thread] Error: {e}")
    #         time.sleep(1.0/cfg['sensor']['fps'])
