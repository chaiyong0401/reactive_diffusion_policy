...
class PoseTrajectoryInterpolator:
    ...
    def schedule_waypoint(self,
        ...
        # curr_time, last_waypoint_time, time을 기준으로 기존 보간기를 잘라.
        start_time = self.times[0] # 기존 보간기의 시작 시간.
        end_time = self.times[-1] # 기존 보간기의 종료 시간
        assert start_time <= end_time
        # print(f"start time1:{start_time}")
        # print(f"end time1:{end_time}")
        ...
        if curr_time is not None:
            if time <= curr_time:
                ...
                # time(target_time)이 curr_time보다 이전이면 변경 없이 현재 보간기를 반환.
                # 그렇지 않다면 start_time을 curr_time과 현재 start_time 중 더 늦은 값으로 설정.
            ...
            if last_waypoint_time is not None:
                ...
                # use start_time
                # print(f"last_waypoint_time: {last_waypoint_time}")
                if time <= last_waypoint_time:
                    # print("time <= last_waypoint_time")
                    end_time = curr_time
                else:
        ...
        # print(f"start time2:{start_time}")
        # print(f"end time2:{end_time}")
        ...
        pos_min_duration = pos_dist / max_pos_speed # =0
        rot_min_duration = rot_dist / max_rot_speed # =0
        duration = max(duration, max(pos_min_duration, rot_min_duration))
        assert duration >= 0
        last_waypoint_time = end_time + duration
        # print(f"last_waypoint_time2:{last_waypoint_time}")
        # insert new pose
        times = np.append(trimmed_interp.times, [last_waypoint_time], axis=0)
        poses = np.append(trimmed_interp.poses, [pose], axis=0)
        # print(f"times:{times}")
