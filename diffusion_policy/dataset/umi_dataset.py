...
class UmiDataset(BaseDataset):
    ...
    def get_normalizer(self, **kwargs) -> LinearNormalizer:
        ...
        for key in self.lowdim_keys:
            ...
            if key.endswith('pos') or 'pos_wrt' in key:
                ...
                this_normalizer = get_range_normalizer_from_stat(stat)
            elif key.endswith('tactile'):  # 07/07
                # tactile signals are typically bounded, use range normalizer
                this_normalizer = get_range_normalizer_from_stat(stat)
            else:
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        for robot_id in range(self.num_robot):
            ...
            # solve relative obs
            obs_pose_mat = convert_pose_mat_rep(    # relative obs_pose_mat
                pose_mat, 
                base_pose_mat=pose_mat[-1],
                pose_rep=self.obs_pose_repr,
                backward=False)
            action_pose_mat = convert_pose_mat_rep(     # relative action_pose_mat
                action_mat, 
                base_pose_mat=pose_mat[-1],
                pose_rep=self.obs_pose_repr,
                backward=False)
