...
class TrainDiffusionUnetImageWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        if cfg.training.resume:
            ...
            # lastest_ckpt_path = pathlib.Path("/home/dyros-recruit/mcy_ws/reactive_diffusion_policy_umi/data/outputs/2025.05.29/23.32.01_train_latent_diffusion_unet_image_real_umi_image_gelsight_emb_ldp_24fps_0529233159/checkpoints/latest.ckpt")
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                if (self.epoch % 1) == 0:
                    with torch.no_grad():
                        for batch in tqdm.tqdm(val_dataloader, desc=f"Validation epoch {self.epoch}"): 
                            ...
                            for i in range(gt_action.shape[0]):
                                log_data.append({
                                    'tcp_pose': obs_dict['tcp_pose'][i].cpu().numpy().tolist(),
