...
class FrankaInterface:
    ...
    def __init__(self, ip='10.0.0.2', port=4242): # local simulation
    ...
    def update_desired_ee_pose(self, pose: np.ndarray):
        ...
        if isinstance(pose, list):  # ← 또는 if not isinstance(pose, np.ndarray):
            pose = np.array(pose)
...
class BimanualFlexivServer():
    ...
    def __init__(self,
        ...
        # init 안에서 설정하는 zerorpc는 gevent 기반 이벤트 루프를 사용하여 multiprocess안에서 동작하지 않음
        # zerorpc를 init이 아닌 run 안에서 정의해야 함 
        # self.zerorpc_client = zerorpc.Client(heartbeat=20)
        # self.zerorpc_client.connect("tcp://147.47.190.17:4242")

        # self.left_robot = FrankaInterface(ip='147.47.190.17', port=4242)
    ...
    def setup_routes(self): # 안전, 통신, 과부하 등 Fault 상황 해제 
        ...
        async def get_current_robot_states() -> BimanualRobotStates:
            ...
            return BimanualRobotStates(
                ...
                # leftRobotTCP=[0.0]*7,
                leftRobotTCP = pose_6d_to_pose_7d(self.left_robot.get_ee_pose()),
                rightRobotTCP=[0.0]*7,
        ...
        async def move_tcp(robot_side: str, request: TargetTCPRequest) -> Dict[str, Any]:
            ...
            try:
                ### example ###############3
                # tcp_xyz = request.target_tcp[:3]
                # tcp_rpy = request.target_tcp[3:]
                # example_tcp = tcp_xyz + [0,0,0]
                # robot.update_desired_ee_pose(example_tcp)
                #####################################
                # target_tcp = [0,0,0,0,0,0]
                # print(target_tcp)
                logger.debug(f"request.target_tcp= {request.target_tcp}")
                robot.update_desired_ee_pose(request.target_tcp)
            ...
            robot.update_desired_ee_pose(np.array(request.target_tcp))
        ...
        async def get_current_tcp(robot_side: str) -> List[float]:
            ...
            try:
                ...
                # client.connect("tcp://147.47.190.17:4242")
                # flange_pose = np.array(client.get_ee_pose())
                # logger.debug(f"client_connect version flange pose: {flange_pose}")
                # client.close()
